...
<script type="text/javascript">
		...
		jQuery(document).ready(
			...
			$(window).scroll(function(){ 
    			...
    			if($(document).height() <= totalheight){ 
     			   if(stop==true){ 
     			       stop=false; 
            		   $.getJSON("/videoapp/VideoListJsonForApp", {pageNo:page, pageSize:20},function(data) {
            			   show = "";
            			   $.each(data, function(id,item) {
            				   show +="<li><a href=\"/videoapp/VideoDetailForApp?videoId="+item.id+"\" data-gaevent=\"home_post_list:home\"><span class=\"title-detail\"><img src=\""+item.source_img+"\" width=\"200\"/>";
							   show +="<h4>"+item.name+"</h4><h4 class=\"news_description\">"+item.description+"</h4></span></a></li>";
            			   }
            			 ); 
	...
	<body>
		...
		<ul class="titles">
			<c:forEach items="${arr}" var="json" varStatus="i">
				<li>
					<a href="/videoapp/VideoDetailForApp?videoId=${json.id}" data-gaevent="home_post_list:home"> <span
						class="title-detail"> <img src="${json.source_img}" width="200"/>
							<h4>
								${json.name}
							</h4>
