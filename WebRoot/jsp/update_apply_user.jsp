 ...
 var reg = new RegExp("^[0-9\.]*$");
 	...
 	var mark="1";
    		$.get(url, {mark: mark,userId: userId,phone: phone,paramName: paramName,paramValue: paramValue},function(data){
    			if (data=="ok"){
    				 alert("更新成功^^"); 
    				 var url="/user/UserDetailAction?phone="+phone;
    		...
    		$.get(url, {mark: mark,userId: userId,phone: phone,province: province,city: city,area: area},function(data){
    			if (data=="ok"){
    				...
    				//window.location.href="/user/GetUserApplyByConditionsAction";
    			 	 ...
    			 	 var t=${phone};
			  		 var url="/user/UserDetailAction?phone="+t;
			    	window.location.href=url;
    			} else {	
    				 alert("更新失败"); 
    			}
...
width:1500px;
  ...
  <input id="hiddenCommunity" type="hidden" name="community">
   ...
   <div class="place">
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li  style="cursor:pointer;"  onclick="javascript:window.history.back(-1);"><span><img src="images/t01.png" /></span> 返回  </li>
    ...
    <table class="tablelist" style="width: 100%;margin-top: 5px;">
       ...
       <c:choose>
        <c:when test="${user.certainStatus=='确认需求'}">
   ...
   <td>来源</td>
        ...
        <tbody>
         <c:forEach items="${jsonArr}" var="user" varStatus="status" >
   <tr>
 
    <td>
   <input style="border:1px solid gray;width:30px;" id="userName${status.index}" type="text" value=" ${user.userName}" onblur="updateValue('${user.userId}','${user.phone}','${status.index}','userName')"/>-
    
     <select name="callName" id="callName${status.index}"  onchange="updateValue('${user.userId}','${user.phone}','${status.index}','callName')">  
			  <option  value="先生" <c:if test="${user.sex=='先生'}"> selected="selected" </c:if> >先生</option>  
			   <option  value="女士" <c:if test="${user.sex=='女士'}"> selected="selected" </c:if> >女士</option>
			    <option  value="小姐" <c:if test="${user.sex=='小姐'}"> selected="selected" </c:if> >小姐</option>
			  <option  value="空" <c:if test="${user.sex==''||user.sex==null}"> selected="selected" </c:if> >空</option>
	</select> 
	</td>
	...
	<td onclick="replaceSelect('${user.userId}','${user.phone}','hiddenUserRemark','${user.userRemark_2}')">
  ...
  <table cellspacing="20px">
  	<tr><td>客服</td><td>确认时间</td></tr>
  	<tr><td>${user.operater}</td><td>${user.certainTime}</td></tr>
  </table>
 </form>
