...
<html>
  ...
  <body>
    <fieldset>
     ...
     <form name="myFrm" action="/ailanqiu/admin/court/GetSchoolList" method="post">
     ...
     <div style="float: left;width:700px;">
  	...
  	<div style="height:40px;margin-top: 20px;">
     	...
     	<option name="lbhayyp"  <c:if test="${userType==null }">selected="selected" </c:if> value="">全部</option>
	  	  ...
	  	  <div style="float: left;margin-left: 30px;">学校类型：
	  	 	 
				
				 <select id="ifOKSchool" name="ifOKSchool">
    
     	<option name="lbhayyp"   <c:if test="${ifOKSchool==0 }">selected="selected" </c:if>     value="0">全部</option>
     	<option name="lbhayyp"  <c:if test="${ifOKSchool==1}">selected="selected" </c:if> value="1">有效</option>
	  	    ...
	  	    <div style="float: left;margin-left: 30px;">学校名称：
	  	 	 <input type="text" value="${schoolName}" name="schoolName">
	  	 </div>	 
									...
									<tr  align="center" id="p${v_user.id}" <c:if test="${status.index%2==0}">bgcolor="#F6F8FA"</c:if>>
										...
										<td>${json.phone}&nbsp;</td>
										 	<td>
												...
												<td>
										 			${json.schoolUserCount}	 
										 		</td>
										 <td><a target="_blank" href="http://www.lanqiure.com/review/share/SchoolShare?userId=${json.userId}&schoolId=${json.schoolId}">分享</a></td>
										  <td><lqr:date DATE="${json.addTime}"></lqr:date></td>
