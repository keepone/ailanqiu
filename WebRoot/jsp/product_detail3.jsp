...
width:1500px;
  ...
  <body onload="Init();">
   <form name="myFrm3" action="user/MatchCompanyAction" method="post">    
    <input type="hidden" id="hidden_productId" value="${json.productId}"> 
   ...
   <tr>
	...
	<table>
		<c:forEach items="${json.thumbnailImgs}" var="thumbnail" varStatus="t">
    	<tr>
    		<td  style="width: 80%;" ><img  width="120px" height="100px;" src="${thumbnail.pic}"/></td>
    		<td  style="width: 5%;" >${thumbnail.pic}</td>
    		<td  style="width: 10%;" >
    			 <form  name="updatethumbnailFrm${t.index}"  method="post" action="/product/UpdatePhotoAction" enctype="multipart/form-data">
				    	 <input type="hidden" name="productId" id="productId" value="${json.productId}"/>
				    	 <input type="hidden" name="detailOrThumbnailId" id="detailOrThumbnailId" value="${thumbnail.id}"/>
				    	 <input type="hidden" name="updateType" id="updateType" value="2"/>
			 				<input type="file" id="userImg" name="resource"/>
			  				<input type="button" value="Submit" onclick="goFrm('updatethumbnailFrm${t.index}')"/>
						</form>
    		</td>
    		<td  style="width: 5%;" >
    		<c:if test="${thumbnail.status==1}">
    		 
    			<input type="radio" name="identity" value="1" checked="checked" onclick="updateImagesStatus('${thumbnail.id}','1','detail')"/>yes
				<input type="radio" name="identity" value="0" onclick="updateThumbnailStatus('${thumbnail.id}','0','thumbnail')"/>no
    		</c:if>
    		<c:if test="${thumbnail.status==0}">
    			<input type="radio" name="identity" value="1" onclick="updateThumbnailStatus('${thumbnail.id}','1','thumbnail')"/>yes
				<input type="radio" name="identity" value="0"  checked="checked"  onclick="updateImagesStatus('${thumbnail.id}','0','detail')"/>no
    		</c:if>	
    		</td>
    	</tr>
    	  </c:forEach>
