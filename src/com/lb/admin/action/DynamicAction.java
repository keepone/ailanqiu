...
@Namespace("/admin/dynamic")
@Controller
@Scope("prototype")
public class DynamicAction extends PageAction{
	...
	public String addLBTeam() {
		...
		Integer status=0;
		 ...
		 if(!img.equals("")){
			 long end=System.currentTimeMillis();
			 int t=(int) ((end-start)/1000);
			 if(t>20&&imgArr.toString().indexOf(".gif")==-1){
		...
		Integer status=0;
		 if(imgURL!=null&&!imgURL.equals("")){
			 if(uploadType==1){
				 ...
				 img=UploadPhotoToQiNiu.uploadToPCByImgURL(imgURL);
			 }else{
				 img=UploadPhotoToQiNiu.uploadToQiNiuByImgURL(imgURL);
			 }
