...
public class NBA98News extends Thread {
	...
	public void run() {
		...
		for(int i=1;i<=2;i++){
			...
			try {		
				System.out.println(Thread.currentThread().getName()+"线程开始停止录入NBA98新闻：将停止2小时,当前i为：－－"+i+",i总共为2，每2小时运行一次，运行4小时");
				this.sleep(7200000);
				System.out.println(Thread.currentThread().getName()+"线程录入NBA98新闻结束停止：");
