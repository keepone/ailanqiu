...
@Component
public class GetNBANewsFromSoHu {
	...
	public static void main(String[] args) {
		String url = "http://sports.sohu.com/nba_a_932.shtml";
		getNewsList_2(url);
	...
	public static void getNewsList(String startURL) {
		...
		try {
			...
			for (Element li : lis) {
				String name = li.text();
				//过滤掉含‘高清’的新闻
				if(!name.equals("")&&name.indexOf("高清")==-1&&name.indexOf("技术统计")==-1&&name.indexOf("直播")==-1&&name.indexOf("数据")==-1){
					...
					if(href.indexOf("group")==-1&&href.indexOf("blog")==-1){
						...
						if(c==0){
								...
								if(json!=null){
									...
									Object[] params={name,content,System.currentTimeMillis(),"搜狐",img,img,"01","0101",href,1};
	...
	public static void getNewsList_2(String startURL) {
		...
		try {
			...
			for (Element li : lis) {
				String name = li.text();
				//过滤掉含‘高清’的新闻
				if(name.indexOf("高清")==-1&&name.indexOf("技术统计")==-1&&name.indexOf("直播")==-1){
					String href = li.attr("abs:href");
					//过滤掉链接含有‘group’的新闻
